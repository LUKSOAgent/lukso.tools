...
async function uploadToIPFSWithNodeFetch(jsonData) {
  const jsonString = JSON.stringify(jsonData);
  
  // Try multiple free IPFS services
  const services = [
    {
      name: 'LUKSO IPFS Node',
      url: 'https://node0.ipfs.lukso.network/api/v0/add?pin=true&quieter=true',
      headers: {}
    },
    {
      name: 'IPFS.io',
      url: 'https://ipfs.io/api/v0/add?pin=true',
      headers: {}
    },
    {
      name: 'Dweb.link',
      url: 'https://dweb.link/api/v0/add?pin=true',
      headers: {}
    }
  ];
...
async function main() {
  ...
  let ipfsResult;
  try {
    ipfsResult = await uploadToIPFSWithNodeFetch(gridData);
    console.log('✅ IPFS upload successful!');
    console.log('IPFS Hash:', ipfsResult.hash);
    console.log('IPFS URL:', ipfsResult.url);
  } catch (error) {
    console.error('\n❌ IPFS upload failed:', error.message);
    console.log('\nTrying manual approach - checking if we can use a temporary solution...');
    
    // Try using a data URL approach for testing (not recommended for production)
    console.log('\n⚠️  IPFS upload is required for LSP28 VerifiableURI');
    console.log('Please provide one of the following environment variables:');
    console.log('  - PINATA_JWT (from pinata.cloud)');
    console.log('  - NFT_STORAGE_KEY (from nft.storage)');
    console.log('\nOr upload the file manually to IPFS and provide the hash.');
    throw error;
  }
