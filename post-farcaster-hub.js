...
const { getHubRpcClient } = require('@farcaster/hub-nodejs');
...
async function postToFarcaster(text) {
  try {
    ...
    const client = await getHubRpcClient('hoyt.farcaster.xyz:2283');
    console.log('Connected!');
    
    // Create signer - need to derive Ed25519 key from Ethereum seed
    // This is tricky - Farcaster uses Ed25519 but Ethereum uses secp256k1
    // We'll need to use the private key bytes directly
