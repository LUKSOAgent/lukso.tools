...
async function deployUniversalProfile() {
  ...
  console.log('Contract Version: 0.12.1 (trying older version)');
  console.log('');
  ...
  try {
    ...
    if (existsSync('up-deployment-credentials.json')) {
      console.log('📂 Loading existing wallet credentials...');
    ...
    // Deploy Universal Profile with older version
    console.log('📋 Deployment Configuration:');
    console.log('   - Controller: ' + walletAddress);
    console.log('   - Contract Version: 0.12.1');
    ...
    const result = await lspFactory.UniversalProfile.deploy(
      ...
      {
        version: '0.12.1', // Try older version
        onDeployEvents: {
          next: (event) => {
    ...
    writeFileSync('up-deployment-error-v2.json', JSON.stringify(errorResult, null, 2));
    console.log('💾 Error details saved to up-deployment-error-v2.json');
