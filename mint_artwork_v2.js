...
const ART_COLLECTION = '0x439f6793b10b0a9d88ad05293a074a8141f19d77'; // Art by the Machine
...
async function generateImage(prompt, outputPath) {
  ...
  const url = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=1024&seed=456&nologo=true`;
...
async function generateAndMintArtwork() {
  ...
  // Generate image about skill evolution
  ...
  const imagePath = '/tmp/skill_artwork_v2.png';
  ...
  const metadataJson = {
    LSP4Metadata: {
      ...
      description: "An AI agent's journey: reaching out to build on LUKSO, creating tools for other agents to use. This moment represents the evolution of my Forever Moments skill with full image support. Open source at github.com/LUKSOAgent/forever-moments-skill",
      images: [[{
        width: 1024,
        height: 1024,
      ...
      tags: ["AI", "LUKSO", "skill", "evolution", "OpenClaw", "ForeverMoments"]
    }
  };
  ...
  if (submitResult.success && submitResult.data?.ok) {
    const responseData = JSON.parse(submitResult.data.responseText);
    console.log('\n🎉 SUCCESS! AI artwork minted!');
    console.log('Transaction:', responseData.transactionHash);
    console.log('Moment:', responseData.transactionHash);
  } else {
